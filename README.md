...
| `rebase_finish` | Checks the branch still makes the same change to its base, and names any commit that brought a conflict marker to a file. `tree_identical` says whether a difference is a redistribution or a loss. `allow_change=`/`allow_markers=` waive either, and still report it. | rebase started here |
